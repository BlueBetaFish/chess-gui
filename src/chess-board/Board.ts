...
export class Board {
    ...
    isCoordinateSafe(square: Coordinate) {
        return (
            0 <= square.x &&
            square.x < (this.boardSize || -100) &&
            0 <= square.y &&
            square.y < (this.boardSize || -100)
        );
    }
        ...
        for (const toSquare of toSquareList) {
            ...
            if (
                ...
                currentPiece?.pieceColor !==
                    this.squares[toSquare.x][toSquare.y].pieceColor
            ) {
                moves.push(
                    new Move(
                        fromSquare,
                        toSquare,
                        currentPiece,
                        this.squares[toSquare.x][toSquare.y]
                    )
                );
            }
        }
                ...
                if (this.enPassant && this.enPassant.equals(toSquare)) {
                    ...
                    moves.push(
                        new Move(
                            ...
                            this.squares[capturedPieceCoordinate.x][
